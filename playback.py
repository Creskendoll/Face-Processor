    ...
    for i, face in enumerate(data):
        if i < len(data)-1:
            ...
            for cur_landmark, next_landmark in zip(current_landmarks, next_landmarks):
                ...
                x_interval = x_diff / n
                y_interval = y_diff / n
                samples = [(c_x+(x_interval*(m-1)), c_y+(y_interval*(m-1))) for m in range(1, n+1)]
            ...
            x_interval = x_diff / n
            y_interval = y_diff / n
            b_sub_samples = [(c_x+(x_interval*(m-1)), c_y+(y_interval*(m-1)), size_diff/2, size_diff/2) for m in range(1, n+1)]
...
def unNormalize(face, dim):
    ...
    offset_y = b_n[1]
    b_size = b_n[2]
    l_n = [[(int((x*b_size)+offset_x), int((y*b_size)+offset_y))
            for (x, y) in l]]
    return (l_n, [b_n])
